...
using System.Collections;
	...
	public static void share(ref ConsoleSystem.Arg arg, ref string[] chatargs) {
		...
		public static Hashtable adminstarterkits = new Hashtable();
		...
		[ConsoleSystem.Admin]
		public static void adminstarter(ref ConsoleSystem.Arg arg) {
			if(config.GetSetting("Commands", "adminstarter") == "false") {
				API.sayUser(arg.argUser.networkPlayer, "This feature has been disabled on this server");
				return;
			}
			bool okay = false;
			if (!adminstarterkits.ContainsKey (arg.argUser.playerClient.userID)) {
				okay = true;
				adminstarterkits.Add (arg.argUser.playerClient.userID, System.Environment.TickCount);
			}
			if (!okay)
				return;
			for(int i=1; i < int.Parse (config.GetSetting("AdminStarterKit", "items") + 1); i++) {
				string[] outer = {
					config.GetSetting("AdminStarterKit", "item" + i + "_name"),
					config.GetSetting("AdminStarterKit", "item" + i + "_amount")
				};
				arg.Args = outer;
				inv.give (ref arg);
			}
			API.sayUser(arg.argUser.networkPlayer, "You have spawned an Admin Starter Kit!");
		}
